...
import {
  Clock, CalendarCheck, Banknote, Users, LineChart,
  Check, ArrowRight, Shield, Headphones, Wallet,
  Monitor, UserCheck, MapPin, FileText, RefreshCw,
  Star, BarChart2, MessageSquare, PieChart, BookOpen,
  Award, Target, Briefcase, Building2, CreditCard,
  Plane, ChevronRight,
} from "lucide-react";
...
export default function FeatureShowcaseCopy() {
  ...
  return (
    <section id="features" className="scroll-mt-24 py-16 lg:py-24 overflow-hidden" style={{ background: "radial-gradient(ellipse 900px 600px at 15% 20%, rgba(99,102,241,0.12) 0%, transparent 65%), radial-gradient(ellipse 700px 700px at 85% 80%, rgba(124,58,237,0.1) 0%, transparent 65%), linear-gradient(135deg, #f5f7ff 0%, #eef2ff 40%, #f0f9ff 70%, #faf5ff 100%)" }}>
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <FadeIn className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-base text-slate-500 lg:text-lg leading-relaxed">
            From hiring to retirement — manage the entire employee lifecycle with
            one connected platform built for UK businesses.
          </p>
        </FadeIn>
            ...
            {features.map((feat, index) => {
              ...
              return (
                <motion.button
                  ...
                  whileHover={!isActive ? { x: 3, boxShadow: "0 4px 20px rgba(99,102,241,0.12)" } : {}}
                ...
                >
                  ...
                  {isActive && (
                    <div className="absolute inset-x-3 top-0 h-px rounded-full" style={{ background: "linear-gradient(90deg, transparent, rgba(255,255,255,0.7), transparent)" }} />
                  )}
                  ...
                  <div className="relative flex items-center gap-3.5 px-5 py-4">
                    ...
                    {isActive ? (
                      /* Progress bar */
                      <div className="h-0.5 w-12 rounded-full overflow-hidden shrink-0" style={{ background: "rgba(255,255,255,0.2)" }}>
                        <motion.div
                    ...
                    {isActive && (
                      ...
                      >
                        <div className="px-5 pb-5">
                          <p className="text-sm leading-relaxed mb-4" style={{ color: "rgba(255,255,255,0.90)" }}>
                            {feat.description}
                          </p>
                          <div className="flex flex-wrap gap-1.5 mb-5">
                            {feat.subFeatures.map((sf) => (
                              ...
                              >
                                <Check className="h-2.5 w-2.5" style={{ color: "rgba(199,210,254,1)" }} strokeWidth={3} />
                                {sf.label}
                              </span>
                            ))}
                          </div>
                          <a
                            ...
                            onMouseLeave={(e) => e.currentTarget.style.color = "rgba(199,210,254,1)"}
                          >
          ...
          <div className="w-full lg:w-[82%] mt-8 lg:mt-0 flex flex-col gap-4">
            ...
            <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/10 aspect-[32/18]">
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <active.icon className={`h-3.5 w-3.5 ${active.lightText}`} strokeWidth={2} />
                  <span className={`text-xs font-semibold ${active.lightText}`}>{active.label}</span>
                </motion.div>
              </AnimatePresence>
